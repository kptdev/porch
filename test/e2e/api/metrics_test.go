...
func (t *PorchSuite) validateSizeMetric(pr *porchapi.PackageRevision, metricName string, selectPodMetrics func(*suiteutils.ParsedMetricsResults) map[string][]suiteutils.MetricResult) {
	...
	if t.UsingDBCache {
		...
		t.Require().GreaterOrEqualf(metric, 1, "Expected metrics to include %q entry with {namespace=%q, package=%q, repository=%q, workspace_name=%q}, but did not", metricName, t.Namespace, pr.Spec.RepositoryName, pr.Spec.PackageName, pr.Spec.WorkspaceName)
