...
func TestIsSecretRefChanged(t *testing.T) {
	tests := []struct {
		name     string
		existing *configapi.Repository
		attempt  *configapi.Repository
		...
		{
			name: "same git secret - no change",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Git: &configapi.GitRepository{
						SecretRef: configapi.SecretRef{Name: "my-secret"},
					},
				},
			},
			attempt: &configapi.Repository{
		...
		{
			name: "git secret changed - change detected",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Git: &configapi.GitRepository{
						SecretRef: configapi.SecretRef{Name: "old-secret"},
					},
				},
			},
			attempt: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Git: &configapi.GitRepository{
						SecretRef: configapi.SecretRef{Name: "new-secret"},
					},
				},
			},
			expected: true,
		...
		{
			name: "git secret added - change detected",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Git: &configapi.GitRepository{},
				},
		...
		{
			name: "git secret removed - change detected",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Git: &configapi.GitRepository{
						SecretRef: configapi.SecretRef{Name: "old-secret"},
		...
		{
			name: "same OCI secret - no change",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Oci: &configapi.OciRepository{
						SecretRef: configapi.SecretRef{Name: "oci-secret"},
					},
				},
			},
			attempt: &configapi.Repository{
		...
		{
			name: "OCI secret changed - change detected",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Oci: &configapi.OciRepository{
						SecretRef: configapi.SecretRef{Name: "old-oci-secret"},
					},
				},
			},
			attempt: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Oci: &configapi.OciRepository{
						SecretRef: configapi.SecretRef{Name: "new-oci-secret"},
					},
				},
			},
			expected: true,
		...
		{
			name: "git present in existing, OCI in attempt - change detected",
			existing: &configapi.Repository{
				Spec: configapi.RepositorySpec{
					Git: &configapi.GitRepository{
						SecretRef: configapi.SecretRef{Name: "git-secret"},
					},
				},
			},
			attempt: &configapi.Repository{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result := isSecretRefChanged(tc.existing, tc.attempt)
			assert.Equal(t, tc.expected, result)
		})
	}
}

// TestUpdateBlocksSecretRefChange verifies UPDATE blocks secret reference changes
func TestUpdateBlocksSecretRefChange(t *testing.T) {
	scheme := newScheme()
	...
	tests := []struct {
		...
		{
			name:     "git secret change blocked",
			existing: makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main"),
			attempted: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
				r.Spec.Git.SecretRef = configapi.SecretRef{Name: "new-secret"}
				return r
			}(),
			expectPass: false,
			message:    "immutable",
		...
		{
			name:     "git secret addition blocked",
			existing: makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main"),
			attempted: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
				r.Spec.Git.SecretRef = configapi.SecretRef{Name: "new-secret"}
		...
		{
			name: "git secret removal blocked",
			existing: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
				r.Spec.Git.SecretRef = configapi.SecretRef{Name: "old-secret"}
				return r
			}(),
			attempted:  makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main"),
			expectPass: false,
			message:    "immutable",
		},
		{
			name: "same git secret allowed",
			existing: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
				r.Spec.Git.SecretRef = configapi.SecretRef{Name: "my-secret"}
				return r
			}(),
			attempted: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
		...
		{
			name: "other field changed but secret same - allowed",
			existing: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
				r.Spec.Git.SecretRef = configapi.SecretRef{Name: "my-secret"}
				r.Spec.Description = "old description"
				return r
			}(),
			attempted: func() *configapi.Repository {
				r := makeRepo("repo1", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
				r.Spec.Git.SecretRef = configapi.SecretRef{Name: "my-secret"}
				r.Spec.Description = "new description"
				return r
			}(),
			expectPass: true,
			message:    "",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			oldData, err := json.Marshal(tc.existing)
			require.NoError(t, err)
			newData, err := json.Marshal(tc.attempted)
			require.NoError(t, err)
			...
			if tc.expectPass {
				...
				assert.False(t, resp.Allowed, "expected denied")
				assert.Contains(t, resp.Result.Message, tc.message, "expected message contains: %s", tc.message)
			}
		})
	}
}

// TestCreateAllowsSecretRef verifies CREATE allows secret references
func TestCreateAllowsSecretRef(t *testing.T) {
	scheme := newScheme()
	client := fake.NewClientBuilder().WithScheme(scheme).Build()
	validator := NewRepositoryValidator(client)

	repo := makeRepo("new-repo", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
	repo.Spec.Git.SecretRef = configapi.SecretRef{Name: "my-secret"}
	...
	resp := validator.Handle(context.Background(), req)
	assert.True(t, resp.Allowed, "CREATE should allow secret references: %s", resp.Result.Message)
}

// TestOCISecretRefBlocking verifies OCI secret refs are also protected
func TestOCISecretRefBlocking(t *testing.T) {
	scheme := newScheme()

	existing := &configapi.Repository{
		ObjectMeta: metav1.ObjectMeta{Name: "oci-repo", Namespace: "ns1"},
		Spec: configapi.RepositorySpec{
	...
	resp := validator.Handle(context.Background(), req)
	assert.False(t, resp.Allowed, "UPDATE should block OCI secret ref changes")
	assert.Contains(t, resp.Result.Message, "immutable")
}

// TestHandleCreateOrUpdateListError verifies that list errors are handled gracefully
func TestHandleCreateOrUpdateListError(t *testing.T) {
	scheme := newScheme()
	...
	validator := NewRepositoryValidator(failingClient)

	repo := makeRepo("test-repo", "ns1", "http://gitea.local/org/repo.git", "dir1", "main")
...
// TestHandleUpdateMalformedOldObject verifies malformed OldObject on UPDATE is rejected
func TestHandleUpdateMalformedOldObject(t *testing.T) {
	scheme := newScheme()
	client := fake.NewClientBuilder().WithScheme(scheme).Build()
	validator := NewRepositoryValidator(client)
	...
	assert.False(t, resp.Allowed)
	assert.Equal(t, int32(http.StatusInternalServerError), resp.Result.Code)
	assert.Contains(t, resp.Result.Message, "failed to unmarshal existing repository")
}
